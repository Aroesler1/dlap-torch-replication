...
5. Not verifiable from the shipped code: the beta-network hyper-parameters ([32,16,8], 2048 epochs, valid-MSE selection); the authors' RtnFcst config is not included. The ensembling choice *is* confirmed: the authors' `sample_checkpoints.zip` contains only the `sharpe/` (best-valid-Sharpe) checkpoint of each trial plus `sdf_normalized_ensemble.npy`, i.e. best-valid-Sharpe checkpoints averaged and L1-normalised, which is what the port does.
