...
for d in sys.argv[1:]:
    ...
    rows.append((os.path.basename(d.rstrip('/')), r['n_seeds'],
                 ...
                 t.get('EV_beta', float('nan')), t.get('XS_R2_beta', float('nan')),
                 t['turnover_mean'], list(t['SR_annual_net_of_cost'].values())[-1], t['max_drawdown']))
hdr = ['model', 'seeds', 'SR train', 'SR valid', 'SR test (monthly)', 'SR test (annual)', 'EV test', 'XS-R2 test', 'turnover', 'net SR (worst cost)', 'max DD']
